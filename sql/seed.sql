...
INSERT IGNORE INTO products
    (category_id, name, kind, main_flower, occasion, delivery_speed, price, availability, is_featured, is_best_seller)
VALUES
    ((SELECT id FROM categories WHERE slug='bouquets'),
        'Rose Romance', 'bouquet', 'Red Roses',      'Anniversary',     'same_day', 2500.00, 'in_stock', 1, 1),
    ((SELECT id FROM categories WHERE slug='bouquets'),
        'Sunshine Mix', 'bouquet', 'Sunflowers',     'Birthday',        'same_day', 1800.00, 'in_stock', 1, 0),
    ((SELECT id FROM categories WHERE slug='bouquets'),
        'Lavender Calm', 'bouquet', 'Lavender',      'Get Well',        'next_day', 1500.00, 'in_stock', 1, 0),
    ((SELECT id FROM categories WHERE slug='bouquets'),
        'Tropical Bliss', 'bouquet', 'Orchids',      'Congratulations', 'same_day', 3200.00, 'in_stock', 1, 1),
    ((SELECT id FROM categories WHERE slug='bouquets'),
        'Garden Fresh', 'bouquet', 'Mixed Seasonal', 'Just Because',    'next_day', 1200.00, 'in_stock', 1, 0);
